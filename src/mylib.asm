...
.equ DISPLAY_RESET_PIN = 4 ; PD4
.equ GREEN_LED_PIN = 6 ; PD6
.equ RED_LED_PIN = 7 ; PD7
...
; default SHT30 address is 0x44
#define SHT30_READ_ADR 0x89 
#define SHT30_WRITE_ADR 0x88
...
; ====
; Reads both humidity and temperature from a SHT30.

; extern char sht30_read(unsigned short *temp,unsigned short *humidity);
; INPUT: r25:r24 - ptr where to store temperature reading
; INPUT: r23:r22 - ptr where to store humidity reading
; RETURN: r24 - 0 = failure, everything else = success
; SCRATCHED: r18,r19,r24,r25
; ====
sht30_read:
          movw r31:r30,r25:r24 ; save pointer so we can store the temp reading later
; send start with SLA+W
	ldi r24,SHT30_WRITE_ADR
	rcall i2c_set_slave_address
	rcall send_start
	brcs error	

; request measurement: Clock stretching, high repeatability
	ldi r24,0x2c
	rcall i2c_send_byte
	brcs error
...
; request read
	ldi r24,SHT30_READ_ADR
	rcall i2c_set_slave_address

	rcall send_start
	brcs error

; receive 	temperature MSB
	rcall i2c_read_ack
	brcs error
          mov r18,r24 ; backup temperature MSB

; receive 	temperature LSB
	rcall i2c_read_ack
	brcs error
          st Z+,r24 ; AVR uses big-endian byte order
          st Z+,r18
...
; TODO: checksum in r24 - verify it !!

; receive 	humidity MSB
	rcall i2c_read_ack
	brcs error
          mov r18,r24 ; backup humidity MSB

; receive 	humidity LSB
	rcall i2c_read_ack
	brcs error
...
; RETURN: r24 != 0 => no errors, r24 = 0 => tx errors
; ====
si7021_reset:
; send start with SLA+W
...
; repeat start with SLA+R
	ldi r24,SI7021_READ_ADR
	rcall i2c_set_slave_address
...
; =====
; Receive one byte via the I2C bus
...
; =====
; Receive one byte via the I2C bus and respond with ACK
; RETURN: r24 - received byte
; RETURN: carry set => error, carry clear => no error
; =====
i2c_read_ack: 
...
; =====
; Receive one byte via the I2C bus and respond with NACK.
; RETURN: r24 - received byte
; RETURN: carry set => error, carry clear => no error
; =====
i2c_read_nack: 
...
; and a number of status codes in TWSRn are possible. 
; Possible status codes in Master mode are 0x38,0x40, or 0x48. 
; The appropriate action to be taken for each of these status codes is detailed in the table
...
; check for transmission error
          lds r24,TWSR
          andi r24, 0xF8 
	cpi r24, 0x10 ; 0x10 = status code: Repeated start transmitted
	breq send_ok
...
; check status
	lds r24,TWSR
	andi r24, 0xF8 
	cpi r24, 0x40 ; 0x40 = SLA+R transmitted, ack received
	breq send_ok2
...
lcd_reset_display:
	ldi r24, LCD_ADR
	rcall i2c_set_slave_address
; wait some time until power has stabilized
        ldi r24,200
        rcall util_msleep
...
; ======
; ADC setup
; INPUT: r24 - input channel selection
; 0000 (0) - ADC0
; 0001 (1) - ADC1
; 0010 (2) - ADC2
; 0011 (3) - ADC3
...
adc_setup:
	andi r24,%00001111
	ori r24 ,%11000000 ; use internal 1.1v reference with cap at Vref pin
	sts ADMUX,r24
...
;                  |||ADC IRQ flag
;                  ||||ADC IRQ enable
;                  |||||ADC prescaler select ( CPU freq/128)
;                  ||||||||
;                  ||||||||
;                  ||||||||     
	ldi r24,%10000111
...
; ====
; Read A/D converter
; RESULT: r24 - low byte , r25 - hi byte
; ====
adc_read:
	lds r24,ADCSRA
	ori r24,%01000000
	sts ADCSRA, r24 ; start conversion
.wait_conversion
	lds r24,ADCSRA
	andi r24,%01000000
...
; ====
; IRQ handler for INT0 interrupts
; ====
.irq 1
extint0_irq:
	reti
; =====
; Enable irq on low level of INT0
; SCRATCHED: r24
; =====
enable_int0:
; disable IRQs
...
; configure INT0 pin as INPUT
	cbi DDRD,1<<2 ; set pin to input
          sbi PORTD,1<<2 ; Enable pull-up resistor
; setup INT0 to trigger on low level
          lds r24,EICRA
          andi r24,~(1<<ISC01|1<<ISC00)
	sts EICRA,r24
; clear pending interrupts
	sbi EIFR,INTF0
; enable IRQ         
...
; ============================
; Makes the MCU enter power-down mode 
; until INT0 goes LOW
; ============================
enter_sleepmode:
	cli
; disable all power-using components (timers,TWI,etc.)
	ldi r24,0xff
	sts PRR,r24
; configure MCU to power-down on SLEEP
          ldi r24,%0101
	out SMCR,r24	
; enable INT0 irq
          rcall enable_int0
; enter standby
          sleep
; disable sleeping on SLEEP instruction
; so we don't accidently fall asleep
          clr r24
	out SMCR,r24
; re-enable all power-using components
	ldi r24,0x00
	sts PRR,r24
          ret
